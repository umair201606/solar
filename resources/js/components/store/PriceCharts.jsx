...
import { formatRs, formatDate, formatDateFull } from "../../lib/format";

// history: array of [isoDate, price] sorted by date ascending.

function pathFrom(points, w, h, pad = 2) {
  ...
  const coords = points.map(([date, price]) => {
    const x = pad + ((new Date(date).getTime() - t0) / tSpan) * (w - pad * 2);
...
export function PriceChart({ history, unit }) {
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between mb-2 text-xs text-gray-400 font-medium">
        <span>
          High: <span className="text-gray-600 font-bold">{formatRs(max, unit)}</span>
        </span>
        <span>
          Low: <span className="text-gray-600 font-bold">{formatRs(min, unit)}</span>
        </span>
      </div>
