...
export default function HeroCards({ overlay = false }) {
  ...
  return (
    <div className={`relative z-30 mx-auto w-full ${overlay ? 'h-full px-2' : 'mt-0 mb-10'}`}>
      <div className={`grid grid-cols-1 lg:grid-cols-[1.9fr_1fr_0.8fr] gap-4 xl:gap-5 ${overlay ? 'h-full' : ''}`}>
        ...
        {overlay ? (
          ...
          <div
            className="w-full relative overflow-hidden shadow-lg h-[220px] sm:h-[300px] rounded-[1.5rem] bg-black group cursor-pointer"
            onClick={openFullscreen}
          >
            <video
              ref={videoRef}
        ...
        {/* Card 2: Stats Grid */}
        <div className={`bg-white p-6 xl:p-8 rounded-[24px] xl:rounded-[2rem] flex flex-col justify-between shadow-[0_4px_6px_2px_rgba(0,0,0,0.1),0_2px_4px_2px_rgba(0,0,0,0.06)] ${overlay ? 'h-full' : 'h-auto min-h-[220px]'}`}>
          <p className="text-[12px] xl:text-[13px] text-gray-600 font-medium leading-relaxed">
          ...
          <div className="flex items-end gap-3 xl:gap-4 mt-6 lg:mt-4 xl:mt-4">
            <div className="flex-1">
              <h4 className="text-[2.2rem] lg:text-[2rem] xl:text-[2.8rem] font-black text-black tracking-tighter leading-none mb-1">+12k</h4>
              <p className="text-[12px] lg:text-[11px] xl:text-[14px] font-bold text-black leading-tight">Solar Panels<br />Installed</p>
            </div>
            <div className="flex-1">
              <div className="w-10 h-10 lg:w-9 lg:h-9 xl:w-12 xl:h-12 bg-[#d4ff00] rounded-[0.8rem] xl:rounded-[1rem] flex items-center justify-center mb-2 xl:mb-3">
                <User className="w-5 h-5 lg:w-4 lg:h-4 xl:w-6 xl:h-6 text-[#041a12]" />
              </div>
              <p className="text-[12px] lg:text-[11px] xl:text-[14px] font-bold text-black leading-tight">Industry Leaders,<br />With Proven<br />Experts</p>
            </div>
          </div>
        </div>

        {/* Card 3: Green Highlight CTA */}
        <div className={`bg-[#d4ff00] p-6 xl:p-8 rounded-[24px] xl:rounded-[2rem] flex flex-col justify-between shadow-[0_4px_6px_2px_rgba(0,0,0,0.1),0_2px_4px_2px_rgba(0,0,0,0.06)] ${overlay ? 'h-full' : 'h-auto min-h-[220px]'}`}>
          <h3 className="font-black text-[1.8rem] lg:text-[1.6rem] xl:text-[2.2rem] text-black tracking-tight leading-[1.05] mb-6">
            Discover Next-Gen Solar Solutions
          </h3>
          <button className="bg-[#041a12] text-white w-max px-6 lg:px-5 xl:px-7 py-3 lg:py-2.5 xl:py-3.5 rounded-full text-[13px] lg:text-[12px] xl:text-[14px] font-bold hover:bg-white hover:text-black transition-colors shadow-lg flex items-center gap-2 mt-auto">
